...
from ..health.infraestructura.cmd.health_cmd import HealthCmd
from ..health.aplicacion.servicios.health_service import HealthService
from ..health.infraestructura.repositorios.health_repository import HealthRepositoryImpl
from ..health.infraestructura.rutas.health_routes import create_health_routes
from ..health.aplicacion.use_cases.health_use_case import HealthUseCase

from ...aplicacion.servicios.auth_service import AuthService
from ..cmd.auth_cmd import AuthCmd
from ..rutas.auth_routes import create_auth_routes
    ...
    def _configure_request_logging(self):
        ...
        def log_response(response):
            ...
            if hasattr(g, 'start_time'):
                ...
                logger.info(f"OUTGOING RESPONSE: {response.status_code} - Duration: {duration:.3f}s - Size: {len(response.get_data())} bytes")
            else:
